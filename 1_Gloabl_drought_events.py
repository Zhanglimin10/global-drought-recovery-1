# Extract characteristics of drought events.
import numpy as np
import time
import os
...
def drought_character(lat, lon, Prep, SMPct, d_event, delay_time, SM_threshold):
    ...
    # 8: Duration of drought recovery (starting from the first day after 'DM' until the first day where SMPct>=20 is exceeded),
    # 9: Precipitation during the drought recovery period (corresponding to the same period as 8),
    # 10: Precipitation on the day following the entire drought period, indicating cumulative precipitation from the day after the drought start date to the day after the drought end date,
