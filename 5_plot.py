...
code = "\n".join(
    [make_html(font) for font in sorted(set([f.name for f in matplotlib.font_manager.fontManager.ttflist]))])
...
for i_rt in range(4):
    ...
    title = RT[i_rt]
    plt.suptitle(title)
    plt.savefig('E:\\phd\\plot\\Results_finial\\Results_2\\' + title + '.jpg') #, dpi=600)
    plt.close()
